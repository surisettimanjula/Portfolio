...
export const DataExportModal: React.FC<DataExportModalProps> = ({ 
  ...
  useEffect(() => {
    ...
    const savedPath = localStorage.getItem('gh_path') || 'constants.ts';
    ...
    try {
      ...
      if (!getResponse.ok) {
        if (getResponse.status === 404) {
           setStatus('error');
           setStatusMsg(`File '${ghConfig.path}' not found in repo. Check the file path.`);
           return;
        }
        throw new Error('Failed to fetch file info. Check your token and permissions.');
      }
      ...
      setStatus('success');
      setStatusMsg('Success! Changes pushed. Your site will update in ~2 minutes.');
        ...
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-xl">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Push your edits to the live website.
            </p>
          </div>
          ...
          {mode === 'auto' ? (
            <div className="space-y-6">
               <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-lg text-sm text-indigo-800 flex gap-3 items-start">
                  ...
                  <div>
                    <strong>One-Click Publish:</strong> <br/>
                    Enter your GitHub details once. Afterwards, just click "Publish" to automatically update your live site.
                  </div>
               </div>
               ...
               <div>
                   <label className="block text-xs font-bold text-gray-700 mb-1">
                     ...
                     <a href="https://github.com/settings/tokens" target="_blank" rel="noopener noreferrer" className="ml-2 text-indigo-600 underline font-normal">Create Token (Classic)</a>
                   </label>
                   <input 
                     type="password"
                     className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                   ...
                   <p className="text-xs text-gray-500 mt-1">
                     Must have <strong>repo</strong> scope (Full control of private repositories).
                   </p>
               </div>
               
               <div>
                  <label className="block text-xs font-bold text-gray-700 mb-1">File Path (Advanced)</label>
                  <input 
            ...
            <div className="space-y-4">
              <div className="mb-4 bg-gray-100 text-gray-700 p-4 rounded-lg text-sm border border-gray-200 shadow-sm">
                <strong>Manual Mode:</strong> Copy the code below and paste it into <code>constants.ts</code> in your GitHub repository manually.
              </div>
